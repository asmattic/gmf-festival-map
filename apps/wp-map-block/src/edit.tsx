		...
		<>
			<InspectorControls>
				<PanelBody title="Map layout" initialOpen={ true }>
					<RangeControl
						...
						onChange={ ( height: number | undefined ) =>
							setAttributes( { height: height ?? 640 } )
						}
					/>
					<ToggleControl
						label="Show legend"
						checked={ attributes.showLegend }
						onChange={ ( showLegend: boolean ) =>
							setAttributes( { showLegend } )
						}
					/>
				</PanelBody>
				...
				<PanelBody title="Map view" initialOpen={ true }>
					<TextControl
						...
						onChange={ ( v: string ) =>
							setAttributes( { centerLat: parseFloat( v ) || 0 } )
						}
					...
					<TextControl
						...
						onChange={ ( v: string ) =>
							setAttributes( { centerLng: parseFloat( v ) || 0 } )
						}
					...
					<RangeControl
						...
						onChange={ ( mapZoom: number | undefined ) =>
							setAttributes( { mapZoom: mapZoom ?? 15 } )
						}
					/>
				</PanelBody>
				...
				<PanelBody title="Live data" initialOpen={ true }>
					<TextControl
						...
						onChange={ ( apiBaseUrl: string ) =>
							setAttributes( { apiBaseUrl } )
						}
						help="Origin that serves /api/map/pois, stages, overlay. Leave empty for same-origin."
					/>
					<p style={ { fontSize: 12, marginTop: 8 } }>
						The block fetches JSON endpoints:{ ' ' }
						<code>…/api/map/pois</code>,{ ' ' }
						<code>…/api/map/stages</code>,{ ' ' }
						<code>…/api/map/overlay</code> (same contract as the
			...
			<div { ...blockProps }>
				...
				>
					...
					>
						OpenStreetMap + Leaflet on the front end; legend &
						filters use the Gasparilla map shell tokens. Configure
						data URL and center above.
					</p>
				</div>
			</div>
		</>
